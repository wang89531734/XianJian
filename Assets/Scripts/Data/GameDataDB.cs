...
public class GameDataDB
{
				...
				for (int i = 1; i <= num; i++)
				{
					...
					if (xmlNode2 != null)
					{
						if (xmlNode2.InnerText.Length != 0)
						{
							...
							{
								//if (Swd6Application.instance.m_NowLanguageType == ENUM_LanguageType.TraditionalChinese)
								//{
								//	GameDataDB.LanguageDB.Add(i, xmlNode2.InnerText);
								//}
								//else
								//{
    ...
    {
        if (OrgString == null || OrgString.Length == 0 || Swd6Application.instance == null)
        {
            return OrgString;
        } 
