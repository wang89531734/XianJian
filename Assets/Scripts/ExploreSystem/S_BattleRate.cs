...
public class S_BattleRate : I_BaseDBF
{
	public int GUID;
	...
	public List<S_BattleGroupRate> Group;

	public S_BattleRate()
	{
		this.Flag = new List<int>();
		this.Group = new List<S_BattleGroupRate>();
	...
	public void ParseJson(string JsonString, IConverter Converter, I_BaseDBF Record)
	{
		...
		for (int k = 0; k < 10; k++)
		{
			S_BattleGroupRate s_BattleGroupRate = new S_BattleGroupRate();
			string key3 = string.Format("Group_{0}", k);
			if (dictionary.ContainsKey(key3))
			{
				string s3 = dictionary[key3];
				int.TryParse(s3, out s_BattleGroupRate.GroupID);
			}
			key3 = string.Format("GroupRate_{0}", k);
			if (dictionary.ContainsKey(key3))
			{
				string s3 = dictionary[key3];
				int.TryParse(s3, out s_BattleGroupRate.GroupRate);
			}
			this.Group.Add(s_BattleGroupRate);
		}
	}
}
