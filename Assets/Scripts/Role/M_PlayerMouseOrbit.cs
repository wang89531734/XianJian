...
public class M_PlayerMouseOrbit : MonoBehaviour
{
	...
	private void UpdateFreeControl()
	{
		...
		if (GameEntry.Input.GetKeyActionPress(KEY_ACTION.CAMERA_LEFT))
		{
			this.m_XAngle -= this.m_XSpeed * 0.01f;
		}
		if (GameEntry.Input.GetKeyActionPress(KEY_ACTION.CAMERA_RIGHT))
		{
			this.m_XAngle += this.m_XSpeed * 0.01f;
		}
		if (GameEntry.Input.GetKeyActionPress(KEY_ACTION.CAMERA_UP))
		{
			this.m_YAngle += this.m_YSpeed * 0.01f;
		}
		if (GameEntry.Input.GetKeyActionPress(KEY_ACTION.CAMERA_DOWN))
		{
			this.m_YAngle -= this.m_YSpeed * 0.01f;
		}
		if (GameEntry.Input.IsPressUpKey())
		{
			vector += this.m_Transform.forward * (this.m_FreeMoveSpeed * Time.deltaTime);
		}
		if (GameEntry.Input.IsPressDownKey())
		{
			vector -= this.m_Transform.forward * (this.m_FreeMoveSpeed * Time.deltaTime);
		}
		if (GameEntry.Input.IsPressLeftKey())
		{
			vector -= this.m_Transform.right * (this.m_FreeMoveSpeed * Time.deltaTime);
		}
		if (GameEntry.Input.IsPressRightKey())
		{
			vector += this.m_Transform.right * (this.m_FreeMoveSpeed * Time.deltaTime);
		}
		if (GameEntry.Input.GetKey(KeyCode.KeypadPlus))
		{
			this.m_FreeMoveSpeed += 0.1f;
		}
		if (GameEntry.Input.GetKey(KeyCode.KeypadMinus))
		{
			this.m_FreeMoveSpeed -= 0.1f;
	...
	private void LateUpdate()
	{
		...
		if (!this.m_LockMode)
		{
			...
			{
				if (GameEntry.Input.GetKeyActionPress(KEY_ACTION.CAMERA_IN))
				{
					this.m_Distance = Mathf.Clamp(this.m_Distance - 0.1f, this.m_DistanceMin, this.m_DistanceMax);
				}
				if (GameEntry.Input.GetKeyActionPress(KEY_ACTION.CAMERA_OUT))
				{
					this.m_Distance = Mathf.Clamp(this.m_Distance - -0.1f, this.m_DistanceMin, this.m_DistanceMax);
				}
			}
