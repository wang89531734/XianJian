...
public class GameFileStream : IDisposable
{
	...
	public void Flush()
	{
		if (this.m_FileMode == FileMode.Create)
		{
			...
			byte[] buffer = this.m_MemoryStream.ToArray();
			if (this.m_HeaderLength != 0L)
			{
				this.m_Stream.Write(buffer, 0, (int)this.m_HeaderLength);
			}
			...
			//gZipOutputStream.Write(buffer, (int)this.m_HeaderLength, count);
			//gZipOutputStream.Close();
			byte[] array = memoryStream.ToArray();
			int count2 = array.Length;
			this.m_Stream.Write(array, 0, count2);
			this.m_Stream.Close();
		}
	}
	...
	{
		long arg_0B_0 = this.m_MemoryStream.Length;
		long arg_17_0 = this.m_MemoryStream.Position;
